...
def h3(svg, text, size=20, bg="#e8f4fd", color="#1a73e8"):
...
<style>
    ...
    .metric-card { background: linear-gradient(135deg, #1a1a2e, #16213e); border: 1px solid #0f3460;
    ...
    .section-header { display:flex; align-items:center; background: linear-gradient(90deg, #0f3460, #16213e);
        padding: 10px 18px; border-radius: 8px; color: white; font-weight: 700; font-size: 1.1rem; margin-bottom: 12px; }
    .result-box { background: linear-gradient(135deg, #0d2137, #163851); border: 1px solid #4fc3f7;
        border-radius: 12px; padding: 20px; color: white; text-align: center; }
    .result-box h2 { color: #4fc3f7; }
    .asl-badge { display:inline-block; background:#0f3460; color:white; border-radius:8px;
        padding:6px 12px; font-weight:bold; font-size:0.95rem; margin:3px; }
...
# ══════════════════════════════════════════════════════════
if page == "Predict":
    st.markdown(f"""<div class="page-title">{icon(ICO_HAND, 30, '#e8f4fd', '#1a73e8')}<h1>ASL Sign Language Detection</h1></div>""", unsafe_allow_html=True)
            ...
            if predict_btn:
                if model is None:
                    ...
                    with col_result:
                        ...
                        fig = go.Figure(go.Bar(x=top5_probs, y=top5_names, orientation='h',
                            marker_color=colors, text=[f"{p:.1f}%" for p in top5_probs], textposition='outside'))
                        fig.update_layout(xaxis_title="Confidence (%)", yaxis=dict(autorange="reversed"),
    ...
    st.markdown("---")
    st.markdown(f'<h3 style="display:flex;align-items:center;">{icon(ICO_GRID, 20, "#e8f4fd", "#1a73e8")} ASL Alphabet Quick Reference</h3>', unsafe_allow_html=True)
...
# ══════════════════════════════════════════════════════════
    ...
    st.markdown(f"""<div class="page-title">{icon(ICO_ABOUT, 30, '#e8f4fd', '#1a73e8')}<h1>About the Project</h1></div>""", unsafe_allow_html=True)
    st.markdown("---")
    col1, col2 = st.columns(2)
    with col1:
        st.markdown(h3(ICO_TARGET, "Objective"), unsafe_allow_html=True)
        st.markdown("""Build a system that can **detect a given ASL input image** and output what the sign represents.
    ...
    with col2:
        ...
        st.markdown("""1. **Data Loading** — Training set with class-labeled subfolders\n2. **Preprocessing** — Resize to 64×64, normalize pixel values [0, 1]\n3. **Model** — Convolutional Neural Network (CNN)\n4. **Training** — Categorical cross-entropy loss, Adam optimizer\n5. **Evaluation** — Accuracy on 29-class test set\n6. **Deployment** — Streamlit web application""")
        st.markdown(h3(ICO_GLOBE, "Real-World Relevance"), unsafe_allow_html=True)
        st.markdown("- **Accessibility Tools** — Real-time ASL translation apps\n- **Communication Bridges** — Assist hearing individuals interpret ASL\n- **Education** — Teach ASL alphabet interactively\n- **Research** — Foundation for full ASL sentence recognition")

    with st.expander("Preprocessing Pipeline"):
        st.code("""img = image.resize((64, 64))
img_array = np.array(img) / 255.0
img_array = np.expand_dims(img_array, axis=0)  # (1, 64, 64, 3)""", language="python")

# ══════════════════════════════════════════════════════════
elif page == "Model Performance":
    st.markdown(f"""<div class="page-title">{icon(ICO_CHART, 30, '#e8f4fd', '#1a73e8')}<h1>Model Performance</h1></div>""", unsafe_allow_html=True)
    st.markdown("---")
    st.info("Metrics from the model trained and evaluated on the ASL dataset.")
    c1, c2, c3, c4 = st.columns(4)
    with c1: st.markdown('<div class="metric-card"><h2>~98%</h2><p>Accuracy</p></div>', unsafe_allow_html=True)
    with c2: st.markdown('<div class="metric-card"><h2>~97%</h2><p>Precision</p></div>', unsafe_allow_html=True)
    with c3: st.markdown('<div class="metric-card"><h2>~98%</h2><p>Recall</p></div>', unsafe_allow_html=True)
    with c4: st.markdown('<div class="metric-card"><h2>29</h2><p>Classes</p></div>', unsafe_allow_html=True)

    st.markdown("<br>", unsafe_allow_html=True)
    col1, col2 = st.columns(2)
    with col1:
        st.markdown(h3(ICO_CPU, "Model Architecture"), unsafe_allow_html=True)
        st.markdown("""| Component | Details |\n|---|---|\n| Base | CNN (Convolutional Neural Network) |\n| Input Shape | 64 × 64 × 3 |\n| Output Classes | 29 |\n| Activation | Softmax |\n| Loss | Categorical Cross-Entropy |\n| Optimizer | Adam |""")
        st.markdown(h3(ICO_LAYERS, "Why CNN for Sign Language?"), unsafe_allow_html=True)
        st.markdown("- CNNs excel at **spatial pattern recognition** — hand shapes, angles, finger positions.\n- Achieves near-human accuracy on static ASL alphabet recognition.")
    with col2:
        st.markdown(h3(ICO_CHART, "Class Distribution"), unsafe_allow_html=True)
        ...
        fig.update_layout(xaxis_title="Class", yaxis_title="Images (approx.)",
            height=300, margin=dict(t=10,b=40), plot_bgcolor='rgba(0,0,0,0)', paper_bgcolor='rgba(0,0,0,0)')
        st.plotly_chart(fig, use_container_width=True)
